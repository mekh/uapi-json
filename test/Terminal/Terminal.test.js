...
const semOrErr = (params, type = 'SEM') => (params.command.match(/^SEM/)
...
const executeCommandMdIssues = sinon.spy((params) => {
    ...
    return {
        '*HFF': getTerminalResponse('set02/HFF-P1'),
        MD: mdCall(),
    }[params.command] || semOrErr(params);
});
...
const executeCommandEmulationFailed = sinon.spy(params => {
    ...
    return params.command === 'I' ? getTerminalResponse('I') : semOrErr(params, 'RESTRICTED');
});
        ...
        it('Should execute command', () => {
            ...
            return uAPITerminal
                ...
                .then(() => {
                    ...
                    expect(logger.log).to.have.callCount(2);
                });
        });

        it('Should execute command with custom stopMD function', () => {
            // Resetting spies
            getSessionToken.resetHistory();
            executeCommandOk.resetHistory();
            closeSession.resetHistory();
