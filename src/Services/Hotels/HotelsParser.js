    ...
    result.Agregators = rateobj['hotel:AggregatorHotelDetails'].map((rate) => {
        ...
        if (rate['hotel:HotelDetailItem']) {
            ...
            rate['hotel:HotelDetailItem'].forEach(detail => {
                let text = detail['hotel:Text'];
                if (text.length === 1) {
                    text = text.join('');
                }
                const name = detail.Name.split(' ').join('');
                detailItem[name] = text;
            });
...
const errorHandler = function (rsp) {
    ...
    throw {
        4965: new HotelsRuntimeError.NoResultsFound(rsp),
        5574: new HotelsRuntimeError.NoEnginesResults(rsp),
        5000: new RequestRuntimeError.UnhandledError(null, new HotelsRuntimeError(rsp)),
    }[code] || new RequestRuntimeError.UnhandledError(null, new HotelsRuntimeError(rsp));
};
