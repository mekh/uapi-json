...
const { RequestRuntimeError } = require('../../Request/RequestErrors');
...
function getTaxInfo(details = {}) {
    return Object.keys(details).map(
        taxKey => Object.assign(
            {
                value: details[taxKey].Amount,
                type: details[taxKey].Category,
            },
            details[taxKey][`common_${this.uapi_version}:TaxDetail`]
                ? {
                    details: details[taxKey][`common_${this.uapi_version}:TaxDetail`].map(
                        taxDetail => ({
...
const countHistogram = arr => {
    ...
    if (Object.prototype.toString.call(result[0]) === '[object Object]') {
        result = result.map(elem => elem.Code);
    }
...
function airFlightInfoRsp(obj) {
    ...
    if (typeof data['air:FlightInfoErrorMessage'] !== 'undefined') {
        const message = data['air:FlightInfoErrorMessage']._;
        const Error = {
            'Airline not supported': AirFlightInfoRuntimeError.AirlineNotSupported,
            'Flight not found': AirFlightInfoRuntimeError.FlightNotFound,
            'Invalid Flight Number field': AirFlightInfoRuntimeError.InvalidFlightNumber,
            undefined: AirFlightInfoRuntimeError,
        }[message];
        throw new Error(obj);
    }

    if (typeof data.Carrier === 'undefined') {
        return data.map(item => fillAirFlightInfoResponseItem(item));
    }
...
function airPrice(obj) {
    ...
    if (priceKeys.length > 1) {
        log.info('More than one solution found in booking. Resolving the cheapest one.');
    ...
    const segments = Object.values(airSegments);
    ...
    const directions = groups.map(leg => {
        const segs = segments.filter(value => value.Group === leg);

        const trips = segs.map(segment => {
            const tripFlightDetails = Object.values(segment['air:FlightDetails']);
    ...
    const taxesInfo = getTaxInfo.call(this, thisFare['air:TaxInfo']);
...
const AirErrorHandler = function (rsp) {
    ...
    throw {
        345: new AirRuntimeError.NoAgreement({ pcc: utils.getErrorPcc(rsp.faultstring) }),
        1512: new AirRuntimeError.UnableToRetrieve(rsp),
        4454: new AirRuntimeError.NoResidualValue(rsp),
        12009: new AirRuntimeError.TicketsNotIssued(rsp),
        13003: new AirRuntimeError.NoReservationToImport(rsp),
        3003: new AirRuntimeError.InvalidRequestData(rsp),
        3000: (() => {
            ...
            if (messages.indexOf('Booking is not complete due to waitlisted segment') !== -1) {
                return new AirRuntimeError.SegmentWaitlisted(rsp);
            }
            // else // unknown error, fall back to SegmentBookingFailed
            return new AirRuntimeError.SegmentBookingFailed(rsp);
        })(),
        2602: new AirRuntimeError.NoResultsFound(rsp),
        3037: new AirRuntimeError.NoResultsFound(rsp),
        undefined: new RequestRuntimeError.UnhandledError(null, new AirRuntimeError(rsp)),
    }[code];
};
...
function getTicketFromEtr(etr, obj) {
    ...
    const allCoupons = ticketsList.map(ticket => Object.entries(ticket['air:Coupon'])
        .map(([couponKey, coupon]) => ({
            key: couponKey,
        ...
        }))).reduce((all, nextChunk) => all.concat(nextChunk), []);
    ...
    const tickets = ticketsList.map(ticket => {
        ...
        const coupons = Object.keys(ticket['air:Coupon']).map(couponKey => {
            ...
            if (airPricingInfo && airPricingInfo['air:FareInfo']) {
                const pricingInfo = airPricingInfo['air:FareInfo'];
                const bookingInfo = airPricingInfo['air:BookingInfo'];
                Object.keys(pricingInfo).forEach(fareKey => {
                    const fare = pricingInfo[fareKey];
                    if (fare.FareBasis === coupon.FareBasis && bookingInfo) {
                        const bInfo = bookingInfo.find(info => info.FareInfoRef === fareKey);
                        bkInfo = bInfo || bkInfo;
                    }
                });
            }
            ...
            return {
                ...
                ...(bkInfo !== null ? { serviceClass: bkInfo.CabinClass } : null)
            };
        });
    ...
    const taxes = (airPricingInfo && airPricingInfo['air:TaxInfo'])
        ? Object.keys(airPricingInfo['air:TaxInfo']).map(taxKey => Object.assign(
            {
...
function airGetTicketsErrorHandler(rsp) {
    ...
    if (code === '3000' && responseHasNoTickets(rsp)) {
        return rsp;
    }

    if (code === '345') {
        throw new AirRuntimeError.NoAgreement({ pcc: utils.getErrorPcc(rsp.faultstring) });
    }
...
function extractBookings(obj) {
    ...
    return record['air:AirReservation'].map(booking => {
        ...
        const serviceSegments = indexedServiceSegments
            ? indexedServiceSegments.map(s => {
                ...
                try {
                    ...
                    log.warn(`PassiveRemark is not service segment: ${remark['passive:Text']}.`);
        ...
        const tickets = (booking['air:DocumentInfo'] && booking['air:DocumentInfo']['air:TicketInfo']) ? (
            booking['air:DocumentInfo']['air:TicketInfo'].map(ticket => ({
        ...
        const pricingInfos = !booking['air:AirPricingInfo']
            ? []
            : Object.keys(booking['air:AirPricingInfo']).map(key => {
                ...
                const baggage = fareInfo && Object.values(fareInfo).map(
                    fareLeg => format.getBaggage(fareLeg['air:BaggageAllowance'])
                );

                const passengersCount = (pricingInfo['air:PassengerType'] || [])
                    .reduce((acc, data) => ({ ...acc, [data.Code]: (acc[data.Code] || 0) + 1 }), {});

                const taxesInfo = getTaxInfo.call(this, pricingInfo['air:TaxInfo']);
...
function importRequest(data) {
    return extractBookings.call(this, data);
}
...
function extractFareRules(obj) {
    ...
    return rulesList.map(item => {
        const result = { ...item };
        const rules = [];
        const listName = result['air:FareRuleLong'] ? 'air:FareRuleLong' : 'air:FareRuleShort';
        result[listName].forEach(rule => {
            const ruleCategoryNumber = parseInt(rule.Category, 10);
            if (rule['air:FareRuleNameValue']) {
                // for short rules
                rules[ruleCategoryNumber] = rule['air:FareRuleNameValue'];
            } else {
                // for long rules
                rules[ruleCategoryNumber] = rule._;
            }
        });

        delete result[listName];
        delete result.FareInfoRef;
        result.Rules = rules;
        return result;
    });
}
...
function exchangeQuote(req) {
    ...
    return utils.deflate(JSON.stringify(token))
        .then(zippedToken => {
            ...
            const segments = Object.values(solution['air:AirSegment']).map(segment => {
                return {
            ...
            const pricingInfo = Object.values(solution['air:AirPricingInfo'])
                .map(pricing => {
                    const bookingInfo = Object.values(pricing['air:BookingInfo'])
                        .map(info => {
...
function availability(rsp) {
    ...
    itinerarySolution['air:AirSegmentRef'].forEach((segmentRef, key) => {
        ...
        if (!availInfo || !availInfo['air:BookingCodeInfo']) {
            return;
        }
